...
class Repository (
    ...
    val cities = localDB.getCities().map { list -> list.map { city -> city.toCity() } }
    ...
    fun add(city: City) = ioScope.launch { fbDB.add(city.toFBCity()) }
    fun remove(city: City) = ioScope.launch { fbDB.remove(city.toFBCity()) }
    fun update(city: City) = ioScope.launch { fbDB.update(city.toFBCity()) }
}